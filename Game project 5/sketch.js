...
function setup() {
	...
	isFalling = false;
	isPlummeting = false;
	treePos_x = [width / 2, width / 2 + 50, width / 2 + 100,
...
function draw() {
	cameraPos_x = gameChar_x - 500;
	noStroke();
	fill(229, 66, 45);
	background(251, 184, 79); // fill the sky orange
	rect(0, floorPos_y, width, height - floorPos_y); // orange ground
	push();
...
function drawTrees() {
	for (var i = 0; i < treePos_x.length; i++) {
		noStroke();
		fill(46, 0, 9);
		ellipse(treePos_x[i] + 20, treePos_y + 110, 10, 80);
		fill(0, 255, 0);
		stroke(5);
		// Corner ellipses
		ellipse(treePos_x[i], treePos_y + 45, 30, 15);
		ellipse(treePos_x[i] + 40, treePos_y + 45, 30, 15);
		ellipse(treePos_x[i], treePos_y + 75, 30, 15);
		ellipse(treePos_x[i] + 40, treePos_y + 75, 30, 15);
		// Top, bottom, left, and right ellipses
		ellipse(treePos_x[i] + 20, treePos_y + 30, 30, 15);
		ellipse(treePos_x[i] + 20, treePos_y + 90, 30, 15);
		ellipse(treePos_x[i] - 10, treePos_y + 60, 30, 15);
		ellipse(treePos_x[i] + 50, treePos_y + 60, 30, 15);
		noStroke();
		circle(treePos_x[i] + 20, treePos_y + 60, 60);
	}
}
function checkCollectable(t_collectable) {
	// Calculates the distance between t_collectable and character
...
function drawCube(xx, x_pos, y_pos) {
	...
	fill(0); 
	stroke(255);
	quad(cW, cH, cW + xx, cH - yy, cW, cH - xx, cW - xx, cH - yy); // Top quad of the cube
	quad(cW, cH, cW + xx, cH - yy, cW + xx, cH + xx, cW, cH + PI * yy); // Right quad of the cube
	quad(cW, cH, cW, cH + PI * yy, cW - xx, cH + xx, cW - xx, cH - yy); // Left quad of the cube
}